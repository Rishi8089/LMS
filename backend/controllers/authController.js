...
export const logout = async (req, res) => {
    try {
        // Use the exact same cookie options as login/register for proper clearing
        const clearOptions = cookieOptions(1); // Same as login
        ...
        // Clear the token cookie with exact matching options
        res.clearCookie("token", clearOptions);

        // Additional fallback clearings for any edge cases
...
export const microsoftAuth = (req, res, next) => {
...
export const microsoftAuthCallback = (req, res) => {
    passport.authenticate('microsoft', { failureRedirect: '/login' }, (err, user) => {
        if (err) {
            console.error('Microsoft auth error:', err);
            return res.redirect('http://localhost:5173/login?error=microsoft_auth_failed');
        }
        if (!user) {
            return res.redirect('http://localhost:5173/login?error=microsoft_auth_failed');
        }
